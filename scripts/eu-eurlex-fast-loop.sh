...
ENVIRONMENT="dev"
TEMPLATE_ID="eur_lex_sparql_regulation_en"
JURISDICTION_ID="jur_eu"
AUTHORITY_ID="auth_eu_eurlex"
MAX_RESOURCES=5
KEEP_SOURCE=0
JSON_OUTPUT=0
DRY_RUN=0
MAX_POLLS=60
POLL_INTERVAL=5
WORKDIR_ROOT="${TMPDIR:-/tmp}/eu-eurlex-fast-loop"
RUN_DIR=""
SOURCE_ID=""
SOURCE_VERSION_ID=""
RUN_ID=""
...
usage() {
  cat <<'EOF'
Usage: scripts/eu-eurlex-fast-loop.sh [options]

Run a narrow EU EUR-Lex preview against platform-control on Cloud Run and verify:
...
expected_title_regex() {
  case "${TEMPLATE_ID}" in
    eur_lex_sparql_regulation_en)
      printf '%s' '(Regulation|REGULATION|GDPR|General Data Protection)'
      ;;
    eur_lex_sparql_directive_en)
      printf '%s' '(Directive|DIRECTIVE|Copyright)'
      ;;
    *)
      printf '%s' '.+'
      ;;
...
VERSION_LABEL="eu-eurlex-fast-loop-$(date -u +%Y%m%dT%H%M%SZ)"
SOURCE_NAME="EU EUR-Lex fast-loop source"
CREATE_PAYLOAD="$(jq -n \
...
resolve_fast_loop_source \
  ...
  "${VERSION_LABEL}" "${TEMPLATE_ID}" "eu" "${CREATE_PAYLOAD}"
...
# EUR-Lex URLs follow the pattern publications.europa.eu/resource/cellar/...
eurlex_url_ok="$(jq -r '[.data[]? | select((.final_url // "") | test("publications\\.europa\\.eu|eur-lex\\.europa\\.eu"))] | length' < "${RUN_DIR}/captured-resources.json")"
...
elif [[ "${title_ok}" -lt 1 || "${eurlex_url_ok}" -lt 1 ]]; then
...
SUMMARY_JSON="$(jq -n \
  ...
  --argjson eurlex_url_ok "${eurlex_url_ok}" \
  '{
    run_mode: $run_mode,
    ...
    checks: {
      ...
      title_ok: $title_ok,
      eurlex_url_ok: $eurlex_url_ok
    }
  }')"

printf '%s\n' "${SUMMARY_JSON}" > "${RUN_DIR}/summary.json"
render_fast_loop_evidence_markdown "${RUN_DIR}/summary.json" "${RUN_DIR}/evidence-summary.md" "EU EUR-Lex"
