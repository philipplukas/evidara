...
ENVIRONMENT="dev"
TEMPLATE_ID="legifrance_codes"
JURISDICTION_ID="jur_fr"
AUTHORITY_ID="auth_fr_legifrance"
MAX_RESOURCES=20
KEEP_SOURCE=0
JSON_OUTPUT=0
DRY_RUN=0
MAX_POLLS=60
POLL_INTERVAL=5
WORKDIR_ROOT="${TMPDIR:-/tmp}/fr-legifrance-fast-loop"
RUN_DIR=""
SOURCE_ID=""
SOURCE_VERSION_ID=""
RUN_ID=""
...
Run a narrow FR Legifrance preview against platform-control on Cloud Run and verify:
  ...
  - minimum French legal-text quality gates
...
expected_title_regex() {
  # French legal code titles
  printf '%s' '(Code|Loi|Décret|Article|Ordonnance|Constitution)'
}
...
VERSION_LABEL="fr-legifrance-fast-loop-$(date -u +%Y%m%dT%H%M%SZ)"
SOURCE_NAME="FR Legifrance fast-loop source"
CREATE_PAYLOAD="$(jq -n \
...
resolve_fast_loop_source \
  ...
  "${VERSION_LABEL}" "${TEMPLATE_ID}" "fr" "${CREATE_PAYLOAD}"
...
source_url_ok="$(jq -r '[.data[]? | select((.final_url // "") | test("legifrance\\.gouv\\.fr"))] | length' < "${RUN_DIR}/captured-resources.json")"
...
render_fast_loop_evidence_markdown "${RUN_DIR}/summary.json" "${RUN_DIR}/evidence-summary.md" "FR Legifrance"
