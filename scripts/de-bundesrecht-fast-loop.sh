...
Run a narrow DE Bundesrecht preview against platform-control and verify:
...
SOURCE_NAME="DE Bundesrecht fast-loop source"
CREATE_PAYLOAD="$(jq -n \
...
resolve_fast_loop_source \
  ...
  "${VERSION_LABEL}" "${TEMPLATE_ID}" "de" "${CREATE_PAYLOAD}"
