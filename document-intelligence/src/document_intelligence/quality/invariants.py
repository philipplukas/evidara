...
def validate_document_and_sections(
    document: Document, sections: Iterable[Section]
) -> None:
    ...
    for section in sections:
        ...
        if section.processing_manifest_id != document.processing_manifest_id:
            raise ValueError(
                "section processing_manifest_id must match parent document"
            )
        if section.provenance.document_id != document.document_id:
            raise ValueError(
                "section provenance document_id must match parent document"
            )
