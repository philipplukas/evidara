...
def resolve_artifact_bundle_event(event_payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    raise EnvelopeError(
        "expected artifact_bundle.available event or Pub/Sub push envelope"
    )
...
def _looks_like_artifact_bundle_event(event_payload: Mapping[str, Any]) -> bool:
    return event_payload.get(
        "event_type"
    ) == "artifact_bundle.available" and isinstance(
...
def _decode_pubsub_push_envelope(pubsub_envelope: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise EnvelopeError(
            "Pub/Sub push envelope message.data was not valid base64"
        ) from error
    ...
    try:
        ...
        raise EnvelopeError(
            "Pub/Sub push envelope message.data was not valid UTF-8"
        ) from error
    except json.JSONDecodeError as error:
        raise EnvelopeError(
            "Pub/Sub push envelope message.data was not valid JSON"
        ) from error

    if not isinstance(decoded_payload, Mapping):
        raise EnvelopeError(
            "Pub/Sub push envelope message.data must decode to an object"
        )
