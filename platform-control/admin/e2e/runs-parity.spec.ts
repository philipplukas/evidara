...
// ---------------------------------------------------------------------------
// Smoke coverage for the ADR-0026 runs parity slice: the operator-action
// stack and keyboard shortcuts wired into `RunList` / `RunShow` using the
...
// `/#/runs` routes, the surface an operator uses.
// ---------------------------------------------------------------------------
...
async function mockRunsApi(page: Page) {
  // Benign catch-all first (lowest priority) — RunDetailSections and other
  // read models the pages touch resolve to an empty 200 instead of hanging.
  ...
  // Single-run getOne feeds RunShow (raw object, not `{ data }`). Registered
  ...
  // Pipeline-health read model for the RunShow detail sections.
...
test.describe("Runs parity (ADR-0026 operator-action stack)", () => {
  ...
  test("pressing O opens the attention run detail", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/#\/runs\/run_running/);
