 ...
 * the workspace's `src/__tests__/ui-alias.test.ts` — a small unit test
 * whose only job is to fail loudly if the path alias to the monorepo-level
 * `styles/ui/` module breaks. tsconfig + vitest + Next bundler all have to
 * stay aligned with `next.config.ts` `turbopack.root`.
 *
