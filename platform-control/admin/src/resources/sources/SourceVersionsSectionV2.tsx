/**
 * `SourceVersionsSectionV2` — Tailwind + ra-core port of the MUI
 * `SourceVersionsSection`, rendered inside the v2 source-detail page
 * (`SourceShowV2`). It owns the full source-version lifecycle surface:
 *
 *   - a lifecycle rollup (counts + next-operator-action) built from the
 *     shared `summarizeSourceVersionLifecycle` helper;
 *   - a version table with edit / compare / preview-run / production-run /
 *     approve / reject affordances (the mutating ones gated behind a
 *     confirm dialog, matching the v1 `ConfirmButton` tiers);
 *   - the create / edit form dialog, whose provider-specific fields turn
 *     into an `acquisition_spec` via the shared `toAcquisitionSpec` helper.
 *
 * All pure form + lifecycle logic is shared with the v1 section through
 * `./sourceVersionForm`; the diff view reuses the already-Tailwind
 * `SourceVersionDiffPanel`. No MUI imports (ADR-0026).
 */
"use client";
...
export function SourceVersionsSectionV2({ source }: { source: SourceRecord }) {
