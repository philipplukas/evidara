/**
 * `SourceCreateV2` — v2 source-create wizard. Uses `ra-core`'s `<Form>` +
 * `useDataProvider`; every input is a Tailwind + radix primitive
 * (`TextInput` / `Select`), so the page has zero MUI imports.
 *
 * Parity with the v1 MUI wizard (`SourceCreate.tsx`): creates the source and
 * its initial source version in one submit via the `source-create-wizard`
 * data-provider resource (`POST /v1/sources/with-version`). The operator
 * picks a country overlay + provider template; the expanded, server-side
 * `acquisition_spec` is previewed live via
 ...
 * `filterAuthoritiesByJurisdiction` — same helper the MUI `AuthoritySelectInput`
 * uses — and validated with `isAuthorityValidForJurisdiction`, so picking a
 * mismatched authority fails at submit with the same message the v1 flow ships.
 * The pure preview/overlay/template helpers live in `./sourceBlueprint`, shared
 * with the v1 page so both render identical summaries.
 */
"use client";
...
export default function SourceCreateV2() {
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      navigate(`/sources-v2/${result.data.id}`);
    } catch (error) {
