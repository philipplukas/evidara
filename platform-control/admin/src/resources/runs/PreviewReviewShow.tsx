/**
 * `PreviewReviewShow` — Tailwind + `ra-core` preview-approval detail page.
 * Renders the preview-run header + operator action stack, a metadata grid, and
 * delegates the lifecycle stack to the shared `RunDetailSections` (same
 * component the runs detail uses). The dataProvider guards the `PreviewReview`
 * getOne to preview-mode runs, so a production run id resolves to a 404 here.
 */
"use client";
...
export default function PreviewReviewShow() {
  const { id } = useParams();
