...
export function describeRunMode(mode: string | null | undefined): RunModeDescriptor {
  // `mode in RUN_MODES` also matches inherited keys — `"constructor" in RUN_MODES`
  // is true — which would return undefined and render an empty badge. Own-property
  // only, so the unknown path is genuinely the fallback it claims to be.
  if (mode && Object.hasOwn(RUN_MODES, mode)) {
