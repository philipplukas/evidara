# Evidara Legal Search — Documentation

## Docs

| Doc | Description |
|-----|-------------|
| [Architecture](./architecture.md) | Component tree, panel layout, data flow diagrams, transition inventory |
| [User Flows](./user-flows.md) | Anchor→Discover→Move model, 5 user journeys with state machine diagrams |
| [Design System](./design-system.md) | Colors, typography, spacing, interactive patterns, icon system |
| [Data Contracts](./data-contracts.md) | View model types, domain→view model mapping, API endpoints, mock→BFF migration |
| [State Management](./state-management.md) | URL + workspace state architecture, provider boundaries, query-cache ownership |
| [State Libraries](./state-libraries.md) | How to choose between `nuqs`, Context, Zustand, Jotai, Redux Toolkit, and React Query |
| [Testing Matrix](./testing-matrix.md) | Declarative journey-to-test-layer mapping for UX/UI interactions |
| [Multi-Country Content Spec](./multi-country-content-spec.md) | End-user content rules for CH/AT/DE/FR/IT scaling (labels, subtitles, overlays, edge states) |

## Will Evolve Naturally

| Layer | Description |
|-------|-------------|
| Component API | Props reference per component, usage examples |
| Developer Guide | Setup, conventions, folder structure, contributing |
| Deployment | Build, CI/CD, environment config |
