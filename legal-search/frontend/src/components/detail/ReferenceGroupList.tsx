...
interface ReferenceGroupListProps {
  references: ReferenceGroup[];
  onFocus?: (id: string) => void;
  /** Omitted where there is no room for the escape hatch (the reading rail). */
...
/**
 * The reference groups, with unresolvable citations marked as such.
 *
 * Extracted from `ReferencesTab` when reading mode grew an evidence rail
 * (#1053): the same citations now render in two places, and the rule that
 * decides whether a citation is a link had to stay in one. A second copy would
 * be the ADR-0052 defect back again in whichever copy got edited second.
 */
export function ReferenceGroupList({
  references,
  onFocus,
  onPivot,
  sourceId,
  sourceTitle,
}: ReferenceGroupListProps) {
