...
`platform-control` is the internal API backend for source lifecycle, run orchestration, approvals, and reference data. Its ops UI is Retool (ADR-0006). The audience for the API is:

1. **Retool** — business action endpoints (approve, reject, trigger run)
2. **document-intelligence** — webhooks reporting run results
3. **Future programmatic access** — CI triggers, external integrations

Because Python is already the language of `document-intelligence` (Databricks) and the ops UI is Retool rather than a custom frontend, FastAPI is the natural fit. It gives automatic OpenAPI generation, strong validation via Pydantic, and aligns the Python runtime across the data side of the platform.
...
The FastAPI API surface is focused on **business actions**, not full CRUD. Retool reads directly from Postgres for list/filter/browse views; the API handles state transitions and webhooks.
...
- **Action-focused API** — Retool handles reads via direct Postgres, so the API surface stays small and purposeful.
...
- Retool must be configured with Postgres read access and platform-control API access as separate resources.
