...
| Path / artifact | Class | Merge owner | Notes |
| --- | --- | --- | --- |
| ``k8s/gitops/{dev,staging,prod}/`` | Product | Infra + service teams | Kustomize roots for Argo **product** app; start with **staging** (see [parallel workstreams](parallel-workstreams.md)). |
...
| Product GitOps Kustomize roots | N/A (new) | ``k8s/gitops/`` in Evidara | Argo **product** app `path:` → `k8s/gitops/<env>`; [staging example](examples/product-argocd-application-staging.template.yaml). |
| *TBD* | | | |
