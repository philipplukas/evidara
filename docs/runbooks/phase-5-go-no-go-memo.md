...
|------|--------|--------|----------------|
| A–C vertical slice | `docs/runbooks/first-vertical-slice-exit-gates.md` | **PASS** (dev smoke ×2 on 2026-04-21) | [tar-64-dev-smoke-2026-04-21.md](evidence/tar-64-dev-smoke-2026-04-21.md) |
...
**CI note:** **Release Readiness** uses GitHub Environment **`RELEASE_READINESS_GITHUB_ENVIRONMENT`** (default **`staging`**) for OIDC; dev-first orgs should set it to **`dev`** so GCP project, DI surfaces, GCS evidence root, DLQ regex, and WIF service account follow the **dev** path (see [Environment strategy](../setup/environment-strategy.md#operator-posture-dev-first-no-staging-gcp-project)). Pair that with **GitHub Environment `dev`** and **`RELEASE_READINESS_E2E_SMOKE_WORKFLOW=E2E Smoke Dev`**. It gates on the latest successful **E2E smoke** workflow named **`RELEASE_READINESS_E2E_SMOKE_WORKFLOW`** (default **`E2E Smoke Staging`**; dev-first: **`E2E Smoke Dev`**). Before rotating either variable, verify the live GitHub environment and workflow names still exist, then run the non-strict Release Readiness dispatch once to confirm the report points at the intended pair. Interaction-flow rows still expect **`Interaction Flow Staging Evidence`** until a dev-native workflow exists. Details: [Runtime stack — §7](runtime-stack.md#7-release-readiness-go-no-go-operation).
