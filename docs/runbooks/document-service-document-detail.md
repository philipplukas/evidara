...
1. **Confirm identity of the document** - `document_id` and optional `document_revision` match a published row or file-backed artifact.
2. **Check BFF logs** - Look for upstream errors calling the Document Service base URL.
3. **Verify `document.processed` was emitted** - Search projection and body reads both depend on publication signals and the published document artifacts.
...
2. **Cold content-dir read path** - File-backed reads can be slower on the first request if the service is cold or the backing store path is newly mounted.
...
2. **Content root** - Check `DOCUMENT_SERVICE_CONTENT_DIR` for file-backed runs or `DI_SURFACES_ROOT_URI` for deployed Delta-backed runs.
...
- Search logs by correlation ID across **frontend -> BFF -> Document Service**.
...
- Whether the document exists in the file-backed document directory or published Delta surface for the requested `document_revision`.
