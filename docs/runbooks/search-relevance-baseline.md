...
- **Internal beta query pack:** for Hetzner staging, use [`scripts/fixtures/internal-beta-staging-queries.txt`](../../scripts/fixtures/internal-beta-staging-queries.txt) and the packet in [`internal-beta-user-flow-evidence.md`](internal-beta-user-flow-evidence.md). This proves the deterministic seed path only; it does not replace a broader corpus relevance baseline.
