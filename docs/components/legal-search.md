...
- **Frontend (Next.js 16):** Full workspace UI with resizable panels, filter panel, result list, detail panel with tabs, mobile layout. 43 components using shadcn/radix primitives and a custom design system. Default runtime uses the live BFF via generated clients; e2e keeps an explicit mock mode for deterministic smoke checks.
  - **Reading is a mode** (#1053). Opening a document lays the desktop workspace out as a reading surface rather than a third panel: the reader takes the centre column at a measured 65–75 characters per line, the outline sits on the left nested by the section `depth` the BFF sends and tracks the reader as it scrolls, the evidence (where the document sits, what it cites, and which citations the corpus cannot answer) sits on the right, and the filter rail collapses to its icon rail because it answers nothing about a document already open. The result list narrows to a strip; retiring it is #1040. The collapse order for narrow windows is a stated rule in `src/lib/reading-layout.ts` — the evidence rail first, then the outline to a strip that still shows the position, never the reader — asserted in `src/__tests__/reading-layout.test.ts` and measured in a browser in `e2e/workspace-panels.spec.ts`.
