...
Every command returns a WorkflowCommandEnvelope JSON document.
"""
    ...
    try:
        ...
        if source_id:
            ...
            source_count = (
                ...
                else sources_payload.get("total", 0)
        ...
        envelope = build_envelope(
            ...
            evidence=ev,
            decision={
                "recommended_action": "propose" if exists else "needs-human",
...
def source_propose(
    ...
    try:
        ...
        if isinstance(sources_payload, list):
            ...
            existing_names = [
                s.get("display_name", "")
                for s in sources_payload.get("items", [])
    ...
    try:
        ...
        envelope = build_envelope(
            ...
            decision={
                ...
                "reason": "Verification complete. Review evidence before approval." if all_passed
...
def source_compensate(
    source_id: Annotated[
        str | None,
        typer.Option("--source-id", help="Source id to compensate (reject or delete)."),
    ] = None,
    run_id: Annotated[
        str | None,
    ...
    try:
        ...
        if source_id:
            # Reject the latest pending version if one exists; otherwise record the intent.
...
def run_evidence(
    ...
    try:
        ...
        try:
            ...
            resource_count = (
                len(resources_payload)
                if isinstance(resources_payload, list)
                else resources_payload.get("total", 0)
