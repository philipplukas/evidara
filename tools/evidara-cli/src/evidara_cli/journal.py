"""Local workflow run journal for Phase 0 of ADR-0022.
...
class WorkflowJournal:
    ...
    def record_step(self, run_id: str, envelope: StepEnvelope) -> WorkflowRun:
        """Append a completed step to the run and update the checkpoint."""
        run = self.get_run(run_id)
        if run is None:
            raise ValueError(f"Workflow run not found: {run_id}")
        attempt = sum(1 for s in run.steps if s.step == envelope.step) + 1
        evidence_dicts = [
            e.to_dict() if isinstance(e, EvidenceItem) else dict(e)
    ...
    def last_step_artifact(
        self, run_id: str, step: str, key: str
    ) -> Any:
