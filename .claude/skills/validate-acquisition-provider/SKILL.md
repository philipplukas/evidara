...
## 1. The two gates, and the question each answers
...
**Use both.** Neither subsumes the other, and for a binary manifestation the
first has to pass before the second is even meaningful.
...
`lexfind_api_provider.py:854-868` is the pattern to copy: it calls the abstaining
...
- `check_capture` has **exactly one** call site in `src` —
  `lexfind_api_provider.py:835-840`.
...
## 3. What neither gate answers — and do not claim it does

**A structurally valid PDF whose text is not law** — a cover sheet, an error
page, a consent interstitial — passes both gates on `main` today.
`artifact_guard.py:35-43` says so in the module docstring and explains why the
fix does not belong there: judging PDF text needs extraction, extraction lives in
...
- `document-intelligence/src/document_intelligence/normalize/pdf.py:169-173` — a
  PDF with no text layer becomes an **empty document, not a failure**. It sets
  `metadata["pdf_no_text_layer"] = True` and returns normally. Nothing on `main`
  reads that flag.
- The text-level half is **open, not merged**: PR #841 (`feat(document-intelligence):
  quarantine a manifestation whose text is not law (#731)`) adds
  `normalize/quarantine.py` with `DI_QUARANTINE_MIN_EXTRACTED_CHARS` (default 200)
  and `DI_QUARANTINE_MIN_LEGAL_MARKERS` (default 3). ADR-0047 is `Proposed`.
  **An open PR resolves nothing.** Check whether it merged before you rely on a
  character floor or a `quarantined` outcome; if it has not, say the gap is open.

So of the four assertions #731 asked for — declared content type, PDF magic
number, marker floor over extracted text, minimum extracted characters — `main`
holds the first two plus a *byte* floor. The two that need extracted text are in
flight. Report that state; do not round it up.
...
(`lexfind_api_provider.py:16-31`): the mirrored file is md5-identical to the
...
apart, with an identifying User-Agent (`_USER_AGENT`, `:94`). What it does **not**
...
- **Never record a skip without a reason.** `lexfind_api_provider.py:841-852` —
...
- **A LIVE code key is not a corpus claim.** `lexfind_api_provider.py:339-340`
...
- [ADR-0047](../../../docs/adr/0047-quarantine-unhandled-document-classes.md) — Proposed; the quarantine taxonomy the DI half implements
